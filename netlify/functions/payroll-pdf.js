/**
 * netlify/functions/payroll-pdf.js  v3
 * - payrolls / employees 분리 쿼리 (join 오류 방지)
 * - ?employeeId=xxx&year=2026&month=2  OR  ?payrollId=xxx 둘 다 지원
 */
...
async function buildPDF(payroll, emp, company) {
  ...
  const n = new Date();
  page.drawText('발행일: ' + n.getFullYear() + '년 ' + (n.getMonth()+1) + '월 ' + n.getDate() + '일',
...
exports.handler = async (event) => {
  ...
  try {
    ...
    let tokenData;
    try { tokenData = verifyToken(authHeader); }
    catch { return { statusCode: 401, headers: CORS, body: JSON.stringify({ success: false, error: '인증 실패' }) }; }

    const q = event.queryStringParameters || {};
    const supabase = getSupabase();

    /* ── 급여 레코드 조회 (join 없이 단순 쿼리) ── */
    let payroll = null;

    if (q.employeeId && q.year && q.month) {
      // 방법 1: employeeId + year + month
      const { data, error } = await supabase
        .from('payrolls')
        .select('*')
        .eq('employee_id', q.employeeId)
        .eq('year', parseInt(q.year))
        .eq('month', parseInt(q.month))
        .single();

      if (error || !data) {
        console.error('[payroll-pdf] payrolls 조회 실패:', error?.message, 'employeeId:', q.employeeId, 'year:', q.year, 'month:', q.month);
        return { statusCode: 404, headers: CORS,
          body: JSON.stringify({ success: false, error: '급여 정보 없음 — 급여 계산을 먼저 실행해주세요' }) };
      }
      payroll = data;

    } else if (q.payrollId) {
      // 방법 2: payrollId 직접 조회
      const { data, error } = await supabase
        .from('payrolls')
        .select('*')
        .eq('id', q.payrollId)
      ...
      if (error || !data) {
        return { statusCode: 404, headers: CORS,
          body: JSON.stringify({ success: false, error: '급여 정보 없음' }) };
      }
      payroll = data;
    ...
    const { data: empCheck } = await supabase
      .from('employees')
      .select('id, name, department, hire_date, bank_name, account_number')
      .eq('id', payroll.employee_id)
      .single();
    ...
    /* ── PDF 생성 ── */
    const pdfBytes = await buildPDF(payroll, empCheck, company);
    const fname = encodeURIComponent(
      '급여명세서_' + payroll.year + '년' + payroll.month + '월_' + (empCheck?.name || '') + '.pdf'
    );
    ...
    return { statusCode: 500, headers: CORS,
      body: JSON.stringify({ success: false, error: 'PDF 생성 실패: ' + err.message }) };
  }
};
