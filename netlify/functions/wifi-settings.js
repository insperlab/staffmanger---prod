...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      let query = supabase
        .from('businesses')
        .select('id, name, is_headquarters, checkin_method, wifi_enabled, wifi_registered_ip, wifi_ip_updated_at, wifi_ip_mismatch_detected, wifi_ip_mismatch_at')
      ...
      return {
        statusCode: 200,
        headers: CORS,
        body: JSON.stringify({ success: true, data: { businesses: data || [], alerts } }),
      };
    }
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      const { error } = await supabase
        .from('businesses')
        .update({ wifi_enabled: false, wifi_ip_mismatch_detected: null, wifi_ip_mismatch_at: null })
        .eq('id', businessId)
        .eq('company_id', companyId);
